...
export default function BackgroundFX({ className = '' }) {
  return (
    <div className={`pointer-events-none absolute inset-0 ${className}`} aria-hidden>
      {/* Deep atmospheric glows - intensity tied to theme */}
      <motion.div
        initial={{ opacity: 0, scale: 0.9 }}
        animate={{ opacity: Number(getComputedStyle(document.documentElement).getPropertyValue('--glow') || 0.65), scale: 1 }}
      ...
      <motion.div
        initial={{ opacity: 0, scale: 0.9 }}
        animate={{ opacity: Math.max(0.3, Number(getComputedStyle(document.documentElement).getPropertyValue('--glow') || 0.55) - 0.1), scale: 1 }}
      ...
      {/* Neon grid - visibility tied to theme */}
      <div
        className="absolute inset-0"
        style={{
          opacity: `var(--grid, 0.06)`,
          backgroundImage:
